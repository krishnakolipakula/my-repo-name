...
public class LLVMCodeGenerator extends delphiBaseVisitor<String> {
    ...
    private void processProcedure(delphiParser.ProcedureDeclContext ctx) {
        ...
        if (ctx.paramList() != null) {
            for (int i = 0; i < ctx.paramList().param().size(); i++) {
                ...
                // allocate local storage for the incoming parameter and store it
                String paramAlloca = var_();
                emit("  " + paramAlloca + " = alloca i32");
                emit("  store i32 %p" + i + ", i32* " + paramAlloca);
                define(pname, paramAlloca);
            }
        }
    ...
    private void processFunction(delphiParser.FunctionDeclContext ctx) {
        ...
        inFunction = true;
            ...
            currentFunctionReturnVar = var_();
            emit("  " + currentFunctionReturnVar + " = alloca i32");
            emit("  store i32 0, i32* " + currentFunctionReturnVar);
    ...
    private void processAssign(delphiParser.AssignStmtContext ctx) {
        ...
        // Check if this is a function return (functionName := value)
    ...
    private void processWhile(delphiParser.WhileStmtContext ctx) {
        ...
        // Convert i32 to i1: icmp ne i32 cond, 0
        String cond_i1 = var_();
    ...
    private String processExpr(delphiParser.ExprContext ctx) {
        ...
        if (ctx instanceof delphiParser.ComparisonContext) {
            delphiParser.ComparisonContext cc = (delphiParser.ComparisonContext) ctx;
            String l = processExpr(cc.expr(0));
            String op = cc.op.getText();
            String r = processExpr(cc.expr(1));
            String res = var_();
            ...
            // icmp returns i1, but we need i32 for compatibility with rest of code
            String cmp_i1 = var_();
            emit("  " + cmp_i1 + " = icmp " + icmpOp + " i32 " + l + ", " + r);
            // Convert i1 to i32 (true -> 1, false -> 0)
    ...
    private String processAtom(delphiParser.AtomContext ctx) {
        ...
        // Function call: ID '(' argList? ')'
        if (!ctx.ID().isEmpty() && ctx.argList() != null) {
            String fname = ctx.ID(0).getText();
            String result = var_();
            StringBuilder call = new StringBuilder();
            call.append("  ").append(result).append(" = call i32 @").append(fname).append("(");
            var args = ctx.argList().expr();
            for (int i = 0; i < args.size(); i++) {
                if (i > 0) call.append(", ");
                String a = processExpr(args.get(i));
                call.append("i32 ").append(a);
            }
            call.append(")");
            emit(call.toString());
            return result;
